...
atoms = Atom[Atom(position, e_dipole) for position=chain(0.1,6)]
...
Ψ₀_atom = basis_ket(basis_atom,2)
...
sz0 = zeros(Complex128, N) .- 1.
sp0 = zeros(Complex128, N)
...
name = "excited_chain_atoms=6_d=01"

plt.figure(figsize=(6,4))
plt.plot(T, -sz_full, lw=3.0, color="darkorange", label="Collective")
plt.plot(T, 2*exp(-γ*T)-1, lw=3.0, color="gray", label="Independent")
